...
const SILVER_CROWN_ICON = "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/pngtree-vector-cartoon-silver-crown-png-image_6096738-F8EHgWhU95y8U3BD9b6pzSzWSLZ1sI-removebg-preview-iU7b9obtznlV2zVPwbPRVn0mTS1yLs.png"

export function SilverProductsSection({ products = [] }: { products?: Product[] }) {
  ...
  const scroll = (direction: "left" | "right") => {
    const container = document.getElementById("silver-items-container")
    if (container) {
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-8">
            <div className="p-2 rounded-lg">
              <Image src={SILVER_CROWN_ICON} alt="Silver Crown" width={48} height={48} className="object-contain" unoptimized />
            </div>
            <h2 className="text-3xl font-light">Сребро</h2>
          </div>
  ...
  return (
    <section className="bg-white py-6 mb-6">
      <button
        ...
        className="w-full flex items-center justify-between py-4 md:py-6 px-6 text-left hover:opacity-70 transition-opacity bg-gradient-to-r from-[#e8eaed] to-[#f5f5f5] shadow-md mb-6"
      >
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg">
            <Image src={SILVER_CROWN_ICON} alt="Silver Crown" width={48} height={48} className="object-contain" unoptimized />
          </div>
          <h2 className="text-xl md:text-2xl font-semibold text-black">Сребро</h2>
        </div>
            ...
            {products.map((product) => {
              ...
              return (
                <div key={product.id} className="flex-none w-[200px]">
                  <div
                    className="bg-[#1a1a2e] rounded-lg p-3 relative group hover:border transition-all flex flex-col border"
                    style={{ height: "380px", borderColor: "#333344" }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = "#a8a8a8"
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = "#333344"
                    }}
                    onTouchStart={(e) => {
                      e.currentTarget.style.borderColor = "#a8a8a8"
                    }}
                  ...
                  >
                    ...
                    <Link href={`/silver/${product.id}`} className="relative aspect-square mb-3 flex-shrink-0 block">
                      <Image
                    ...
                    <Link href={`/silver/${product.id}`}>
                      <h3
                        className="text-xs md:text-base font-medium mb-1 line-clamp-2 min-h-[2.25rem] md:min-h-[2.5rem] hover:text-[#c0c0c0] transition-colors leading-tight"
                        style={{ color: "#ffffff" }}
                      >
                        {product.name}
                      </h3>
                    ...
                    {hasPromotion ? (
                      <div className="mb-2 mt-auto flex items-baseline gap-1 flex-wrap">
                        ...
                        <span className="text-lg font-bold text-[#c0c0c0]">{finalPrice.toFixed(2)} €</span>
                      </div>
                    ) : (
                      <div className="text-lg font-bold mb-2 text-[#c0c0c0] mt-auto">{finalPrice.toFixed(2)} €</div>
                    )}
                    ...
                    >
                      <div
                        ...
                        style={{ backgroundColor: "#222233", width: "40px", height: "36px" }}
                      >
                        <ShoppingCart className="w-4 h-4" style={{ color: "#c0c0c0" }} />
                      </div>
                      <div
